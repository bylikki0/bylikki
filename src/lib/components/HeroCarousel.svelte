<script lang="ts">
	...
	const featured = $derived(products[index]);
	...
	const geo = (i: number) => geometry[(i - index + slides.length) % slides.length];
...
>
	...
	<div class="relative z-[7] flex flex-col gap-3 lg:absolute lg:top-[104px] lg:left-[70px] lg:w-[452px] lg:gap-[18px]">
		...
		<div class="flex flex-wrap items-center gap-4 lg:mt-1.5 lg:gap-[18px]">
			<ChunkyButton class="w-full lg:w-auto">{slide.cta}</ChunkyButton>
	...
	<div class="absolute top-0 -right-[60px] hidden h-[820px] w-[940px] lg:block">
		{#each products as product, i (product.slug)}
			<button
				onclick={() => (index = i)}
				class="absolute top-[48%] left-1/2 h-[520px] w-[396px] cursor-pointer text-left transition-[transform,opacity] duration-[800ms] [transition-timing-function:cubic-bezier(.4,0,.2,1)]"
