...
<div class="grid grid-cols-1 lg:min-h-[820px] lg:grid-cols-2">
	...
	>
		...
		<Star color="#FFDE59" size={150} class="absolute top-[130px] -right-[30px] hidden animate-float lg:block" />
		<div class="absolute -right-20 -bottom-[90px] z-0 hidden h-[230px] w-[230px] rounded-full bg-blue-soft lg:block"></div>
	</section>
		...
		<div class="flex w-full max-w-[420px] flex-col gap-[22px]">
			{#if step === 'email'}
				<div class="flex flex-col gap-2">
					<span class="text-[12px] font-semibold tracking-[0.16em] text-pink uppercase">Étape 1 / 2</span>
				...
				<form class="flex flex-col gap-[22px]" onsubmit={(e) => (e.preventDefault(), sendCode())}>
					<div class="flex flex-col gap-2">
						...
						/>
						{#if emailError}
							<span class="text-[13px] font-semibold text-pink-deep">Il manque un @ quelque part.</span>
						{/if}
					</div>
				...
				<p class="m-0 text-[12.5px] leading-[1.55] text-ink/60">
					...
					<a href="/legal?doc=rgpd">politique de confidentialité</a>. Aucun e-mail promotionnel sans ton
					accord.
				</p>
			{:else if step === 'otp'}
				<div class="flex flex-col gap-2">
					<span class="text-[12px] font-semibold tracking-[0.16em] text-pink uppercase">Étape 2 / 2</span>
			...
			{:else}
				<div class="flex flex-col items-start gap-5">
					...
					<button onclick={backToEmail} class="cursor-pointer border-b-[1.5px] border-ink pb-px text-[13.5px]">
						Rejouer le flow
					</button>
				</div>
			{/if}
